...
import { createBidder, createAuctioneer, createSupport } from '@/lib/features/user/userSlice';
...
export default function RegisterModal({ onClose }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (formData.user_type === 'bidder') {
            dispatch(createBidder(formData));
        } else if (formData.user_type === 'auctioneer') {
            dispatch(createAuctioneer(formData));
        } else {
                ...
                <form className="space-y-4" onSubmit={(e) => { handleSubmit(e) }}>
                    <div className="grid md:grid-cols-2 md:gap-6">
                        ...
                        <div className="relative z-0 w-full mb-5 group">
                            ...
                            />
                            <label htmlFor="first_name" className="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-brand peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Nombre</label>
                        </div>
                        {/* LAST NAME */}
                        <div className="relative z-0 w-full mb-5 group">
                            <input
                    ...
                    <div className="relative z-0 w-full mb-5 group">
                        <input
                            ...
                            onChange={handleChange}
                        />
                        <label htmlFor="phone" className="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-brand peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Número de teléfono (4127795032)</label>
                    </div>
                    {/* USER TYPE */}
